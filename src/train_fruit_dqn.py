...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train from-scratch DQN agent for Fruit Cutter")
    ...
    parser.add_argument("--save-dir", type=str, default="models/fruit")
    ...
    env = FruitCutterEnv(config=config, seed=args.seed, state_grid_size=state_grid_size)
    ...
    print(
        f"Fruit training config | grid={args.grid_size} state_grid={state_grid_size} "
    ...
    for episode in range(1, args.episodes + 1):
        ...
        print(
            f"ep={episode:4d} steps={steps:4d} score={int(info['score']):4d} sliced={total_sliced:3d} missed={total_missed:3d} "
        ...
        if episode % args.eval_every == 0:
            eval_env = FruitCutterEnv(
                config=FruitCutterConfig(grid_size=args.grid_size, spawn_prob=args.spawn_prob, bomb_prob=args.bomb_prob),
                seed=args.seed + 999,
                state_grid_size=state_grid_size,
            )
            stats = evaluate_fruit_policy(eval_env, online, episodes=args.eval_episodes, seed_start=args.seed + 7000)
            print(
                f"  eval: avg_score={stats.avg_score:.2f} median={stats.median_score:.2f} "
                f"avg_steps={stats.avg_steps:.2f} avg_sliced={stats.avg_sliced_per_episode:.2f} avg_missed={stats.avg_missed_per_episode:.2f}"
            )
            if stats.avg_score > best_eval:
                best_eval = stats.avg_score
                best_path = save_dir / "fruit_dqn_best.json"
                online.save(best_path)
                print(f"  saved new best checkpoint: {best_path}")

    final_path = save_dir / "fruit_dqn_final.json"
