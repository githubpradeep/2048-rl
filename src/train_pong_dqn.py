...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train from-scratch DQN agent for Pong")
    ...
    parser.add_argument("--save-dir", type=str, default="models/pong")
    ...
    env = PongEnv(config=config, seed=args.seed)
    ...
    print(
        f"Pong training config | board={args.width}x{args.height} paddle={args.paddle_height} "
        f"opp_track={args.opponent_track_prob:.2f} double_dqn={args.double_dqn} dueling={args.dueling}"
    )
    ...
    for episode in range(1, args.episodes + 1):
        ...
        opp_points = 0
        rallies = 0
        info = {"score": 0, "opponent_score": 0, "lives": config.start_lives}
        ...
        while not done and steps < args.max_steps:
            ...
            player_points += 1 if bool(info["player_scored"]) else 0
            opp_points += 1 if bool(info["opponent_scored"]) else 0
            rallies += 1 if bool(info["player_hit"]) else 0
        ...
        print(
            f"ep={episode:4d} steps={steps:4d} score={int(info['score']):3d}-{int(info['opponent_score']):3d} "
            f"lives={int(info['lives']):2d} hits={rallies:3d} reward={episode_reward:8.2f} "
        ...
        if episode % args.eval_every == 0:
            eval_env = PongEnv(config=config, seed=args.seed + 999)
            stats = evaluate_pong_policy(eval_env, online, episodes=args.eval_episodes, seed_start=args.seed + 7000)
            print(
                ...
                f"avg_opp_score={stats.avg_opponent_score:.2f} avg_hits={stats.avg_player_hits:.2f}"
            )
            if stats.avg_score > best_eval:
                best_eval = stats.avg_score
                best_path = save_dir / "pong_dqn_best.json"
                online.save(best_path)
                print(f"  saved new best checkpoint: {best_path}")

    final_path = save_dir / "pong_dqn_final.json"
